...
@objc public protocol PulleyDelegate: class {
    ...
    @objc optional func drawerPositionDidChange(drawer: LocationShareViewController)
    @objc optional func drawerPositionDidComplete(drawer: LocationShareViewController)
    @objc optional func makeUIAdjustmentsForFullscreen(progress: CGFloat)
    @objc optional func drawerChangedDistanceFromBottom(drawer: LocationShareViewController, distance: CGFloat)
}
...
public enum PulleyPosition: Int {
    ...
    public static func positionFor(string: String?) -> PulleyPosition {
        ...
        default:
            print("LocationShareViewController: Position for string '\(positionString)' not found. Available values are: collapsed, partiallyRevealed, open, and closed. Defaulting to collapsed.")
            return .collapsed
        }
    }
}
...
open class LocationShareViewController: UIViewController
{
    ...
    fileprivate let drawerScrollView: LocationSharePassthroughScrollView = LocationSharePassthroughScrollView()
    ...
    private func setUpLayout()
    {
        ...
        dimmingViewTapRecognizer = UITapGestureRecognizer(target: self, action: #selector(self.dimmingViewTapRecognizerAction(_:)))
    ...
    {
        self.navigationItem.title = LMConstant.SendLocation
        self.addNavigationBarbuttons(true);
        NotificationCenter.default.addObserver(self, selector: #selector(self.shareLocation(_:)), name: NSNotification.Name(rawValue: LMConstant.kShareLocationNotification), object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(self.openOrCloseSearchStatus(_:)), name: NSNotification.Name(rawValue: LMConstant.kOpenOrCloseSearchStatusNotification), object: nil)
    ...
    private func updateDidLayoutSubview()
    {
        // Layout main content
        ...
        let lowestStop: CGFloat = [(self.view.bounds.size.height - topInset), collapsedHeight, partialRevealHeight].min() ?? 0
    ...
    private func addNavigationBarbuttons(_ isShow : Bool)
    {
        if isShow
        {
            self.navigationItem.leftBarButtonItem = UIBarButtonItem(barButtonSystemItem: .cancel, target: self, action: #selector(self.btnCancelPressed(_:)))
            self.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .refresh, target: self, action: #selector(self.btnRefreshPressed(_:)))
        }
        else
        {
    ...
    {
        let isCurrentLocation = (self.drawerContentViewController as! DrawerContentViewController).isCurrentLocation;
        self.dismiss(animated: true) {
            if let handler = LocationShareManager.shareLocationCompletionHandler
            {
                let responseDic = notification.object  as?  NSMutableDictionary
                responseDic?.setValue(LocationManager.getGoogleSaticImage(responseDic?.value(forKey: kPlaceLatitude) as! Double, longitute: responseDic?.value(forKey: kPlaceLongitude) as! Double), forKey: kPlaceSataticImage)
                handler!(responseDic!, isCurrentLocation, false);
            }
            else{}
        }
    }
    
    @objc func openOrCloseSearchStatus(_ notification: Notification)
    {
        DispatchQueue.main.async {
            self.addNavigationBarbuttons(notification.object as! Bool);
        }
    }
    ...
    public func getCollapsedDrawerHeight() -> CGFloat
    {
        var collapsedHeight : CGFloat = kPulleyDefaultCollapsedHeight
        if let drawerVCCompliant = drawerContentViewController as? PulleyDrawerViewControllerDelegate
        {
    ...
    public func getPartialRevealHeight() -> CGFloat
    {
        var partialRevealHeight : CGFloat = kPulleyDefaultPartialRevealHeight
        if let drawerVCCompliant = drawerContentViewController as? PulleyDrawerViewControllerDelegate
        {
    ...
    public func setDrawerPosition(position: PulleyPosition, animated: Bool = true)
    {
        guard supportedDrawerPositions.contains(position) else {
            ...
            print("LocationShareViewController: You can't set the drawer position to something not supported by the current view controller contained in the drawer. If you haven't already, you may need to implement the PulleyDrawerViewControllerDelegate.")
            return
        }
        ...
        let lowestStop: CGFloat = drawerStops.min() ?? 0
    ...
    @objc func btnCancelPressed(_ sender : UIButton)
    {
        ...
        self.dismiss(animated: true){
            if let handler = LocationShareManager.shareLocationCompletionHandler
            {
                ...
                handler!(dict, isCurrentLocation, true);
            }
        }
    }
    
    @objc func btnRefreshPressed(_ sender : UIButton)
    {
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: LMConstant.kRefreshMapViewNotification), object: nil);
    }
    
    @objc func dimmingViewTapRecognizerAction(_ gestureRecognizer: UITapGestureRecognizer)
    {
...
extension LocationShareViewController: LocationSharePassthroughScrollViewDelegate {
...
extension LocationShareViewController: UIScrollViewDelegate {
